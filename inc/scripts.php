...
function kasimir_scripts() {
	...
	// Enqueue scripts.
	wp_enqueue_script( 'kasimir-scripts', get_template_directory_uri() . '/assets/js/project' . $suffix . '.js', array( 'jquery' ), $version, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'kasimir_scripts' );
...
if ( class_exists( 'WDS_Simple_Page_Builder' ) && version_compare( WDS_Simple_Page_Builder::VERSION, '1.6', '>=' ) ) :
	 ...
	 */
	function kasimir_enqueue_page_builder_scripts() {
	...
	add_action( 'wds_page_builder_after_load_parts', 'kasimir_enqueue_page_builder_scripts' );
 ...
 */
function kasimir_include_svg_icons() {
